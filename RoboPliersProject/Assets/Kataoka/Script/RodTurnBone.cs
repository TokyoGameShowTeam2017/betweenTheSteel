    ...
    public class RodTurnBone : MonoBehaviour
    {
        ...
        //回転速度で曲げる（軸設定版）
        public void RotateAxisVelo(Vector3 axis, float veloAngle,int index)
        {
            //int count = 0;

            //while (true)
            //{
            //    if (!mBones[index].GetComponent<RodTurnBone>().GetIsMaxRotate() && maxFlag)
            //    {
            //        break;
            //    }
            //    if (m_RodTurn.GetComponent<Rod>().m_point == Rod.StartPoint.LEFT_POINT)
            //    {
            //        index++;
            //        count++;
            ...
            //    if (index <= 0 || mRotatePoints.Count - 1 < index || count >= 4) return;
            //}


            mRotatePoints[index].transform.Rotate(axis, veloAngle);
            //回転情報を保存
            mBoneRotate = mRotatePoint.transform.rotation;
        }
